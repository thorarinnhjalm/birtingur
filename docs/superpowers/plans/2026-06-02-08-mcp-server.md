...
**Architecture:** Vercel HTTP-mode MCP server at `mcp.birtingur.app`. Each tool is a thin wrapper around a REST endpoint. Authentication uses MCP-native bearer tokens that map to platform service-account API keys. Tool descriptions are bilingual (Icelandic primary, English secondary).
...
```ts
const API_BASE = process.env.ADA_API_BASE ?? 'https://api.birtingur.app';
