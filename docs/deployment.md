...
2. **Authentication:**
   ...
   * Undir *Authorized Domains* skaltu bæta við léninu þínu (t.d. `birtingur-dashboard.vercel.app` eða `app.adplatform.is`).
3. **Cloud Firestore:**
   * Stofnaðu Firestore gagnagrunn í *production mode*.
...
5. **Þjónustuaðgangur (Service Account):**
   * Farðu í *Project Settings* -> *Service Accounts*.
