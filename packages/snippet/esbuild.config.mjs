...
await build({
  ...
  define: {
    ...
    // (`serve.adplatform.is`) does not exist in DNS, so the snippet deployed
    // to production loaded fine and then sent every request into a black
    // hole. Nothing errored anywhere — the publisher just saw no ad and the
    // platform recorded no traffic.
    //
